...
fig, axes = plt.subplots(2, 2, figsize=(16 / 2.54, 13 / 2.54), sharex=True)
...
fig.tight_layout()
fig.show()
fig.savefig('cr_modeling_pseudosections.pdf', dpi=300)
...
fig, axes = plt.subplots(3, 2, figsize=(16 / 2.54, 12 / 2.54))
# pg.show(mesh, data=np.real(rho), ax=axes[0], label=r"$\rho'~[\Omega m]$")
# pg.show(mesh, data=np.imag(rho), ax=axes[1], label=r"$\rho''~[\Omega m]$")
pg.show(
    mesh, data=np.abs(rho), ax=axes[0, 0], label=r"$|\rho|~[\Omega m]$",
...
# phia is stored in radians
ert.showERTData(
    data, vals=data['phia'] * 1000, label=r'$\phi_a$ [mrad]', ax=axes[1, 1],
    orientation='vertical',
    cMap='rainbow_r',
)

from egu_cr_inv import plot_inversion

plot_inversion(axes[2, :])

for ax in axes[[0, 2], :].flat:
    ax.set_xlim(-5, 55)
    ax.set_ylim(-25, 0)
    for s in scheme.sensors():
        ax.scatter(s[0], s[1], color='k', s=5, clip_on=False)

axes[0, 0].set_ylabel('z [m]')
axes[0, 0].set_title('Forward model', loc='left', pad=10)
axes[1, 0].set_title('Synthetic data', loc='left', pad=10)
axes[2, 0].set_title('Proof-of-concept complex inversion', loc='left', pad=10)
axes[2, 0].set_ylabel('z [m]')
axes[2, 0].set_xlabel('x [m]')
axes[2, 1].set_xlabel('x [m]')
fig.tight_layout()
fig.show()
fig.savefig('cr_modeling.pdf', dpi=300, bbox_inches='tight')
